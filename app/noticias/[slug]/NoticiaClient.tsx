  ...
  useEffect(() => {
    ...
    const loadArticle = async () => {
      try {
        ...
        console.log('Imágenes del artículo:', {
          imageUrl: foundArticle.imageUrl,
          images: foundArticle.images,
          imagesLength: foundArticle.images?.length || 0
        });
        const allNews = await getNews();
        console.log('Artículo encontrado:', foundArticle);
        console.log('Categoría del artículo:', foundArticle.category);
        console.log('Todas las noticias:', allNews);
        const related = (allNews || []).filter((a) => normalizeCategory(a.category) === normalizeCategory(foundArticle.category) && a.id !== foundArticle.id).slice(0, 3);
        console.log('Noticias relacionadas:', related);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <aside className="space-y-8">
          ...
          <div>
            ...
            <div className="space-y-4">
              {relatedNews.length > 0 ? (
                ...
                <p className="text-muted-foreground text-sm">No hay noticias relacionadas en esta categoría.</p>
              )}
            </div>
          </div>
          ...
          {/* Banner/Ad Space */}
          <Card className="bg-muted border-2 border-dashed border-muted-foreground/20">
            <CardContent className="p-8 text-center">
              <p className="text-sm text-muted-foreground mb-4 font-medium">Espacio Publicitario</p>
              <div className="aspect-square bg-background rounded-lg flex items-center justify-center border border-muted-foreground/30">
