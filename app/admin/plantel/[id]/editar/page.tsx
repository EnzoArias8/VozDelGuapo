...
import { getPlayerById, updatePlayer, Player } from "@/lib/data-manager"
...
export default function EditarJugadorPage() {
  ...
  useEffect(() => {
    const loadedPlayer = getPlayerById(playerId) || mockPlayers.find(p => p.id === playerId)
    if (loadedPlayer) {
  ...
  const handleSave = () => {
    ...
    const success = updatePlayer(player.id, formData);
    if (success) {
